...
int recursiveBinSearchHelper(const std::vector<Book>& books, const Book& target,
                             int low, int high) {
  if (low > high) {
    return 0;  // Not found
  }
...
int recursiveBinSearch(const std::vector<Book>& books,
                       const std::vector<Book>& requests) {
  auto counter = 0;
...
// std::string binarySearch(std::vector<std::string>& books, std::string&
// request){ sort books
